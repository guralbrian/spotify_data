...
temp.df <- read.csv("data/raw/listen_history_1year.csv", row.names = 1)
```
...
final.df <- merged.df %>%
  mutate(
    percent_played = (msPlayed / duration_ms) * 100,
    skipped = ifelse(percent_played < 50, "Yes", "No"),
...
```{r plot heatmap, fig.width=20, fig.height=8}
...
temp.df <- temp.df %>%
  mutate(endTime = ymd_hms(endTime, tz = "UTC") %>% with_tz("America/New_York"))
temp.df$time_of_day <- lubridate::hour(temp.df$endTime) + lubridate::minute(temp.df$endTime) / 60
temp.df$day_of_week <- weekdays(temp.df$endTime)


temp.df$day_of_week <- factor(temp.df$day_of_week, levels = c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"))
...
ggplot(temp.df, aes(x = time_of_day, fill = day_of_week)) +
