...
```{r load data}
df <- read.csv("data/raw/individuals/garias/listen_history_1year.csv", row.names = 1)
```
```{r data cleaning}
df <- df %>%
...
w.artist + plot_annotation(title = "Gabo Babo's Top Artists",
...
w.person + plot_annotation(title = "Gabo Babo's favorite music and listening times",
                           theme = theme(plot.title = element_text(size = 25)))
```
