...
name <- "brian"
id <- 'bgural'
df <- read.csv(paste0("~/data/raw/individuals/", id, "/extended/MyData/listen_history_b.csv"), row.names = 1)
```
```{r data cleaning}
...
p.total.artist <- total.ip |> 
  ...
  coord_flip() +
  scale_fill_manual(values = pal) +
  scale_x_discrete(limits = rev(levels(total.artist$ip_addr_decrypted))) + 
  xlab("IP Address") +
  ylab("Total Hours Listened") +
